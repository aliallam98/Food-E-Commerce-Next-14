...
const variants = {
  base: 'relative rounded-md aspect-square flex justify-center items-center flex-col cursor-pointer min-h-[150px] min-w-[150px] border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out',
  image:
...
const MultiImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div className=''>
        <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,150px))] gap-4">
          {/* Images */}
          {value?.map(({ file, progress }, index) => (
            <div
              key={index}
              className={variants.image + ' aspect-square h-[150px]'}
            >
              <Image
                width={150}
                height={80}
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            ...
            >
              {/* Main File Input */}
              <input className='h-20' ref={ref} {...getInputProps()} />
